...
$.getJSON("./json/recepten.json", function(data){
    ...
    $.each(data, function(key, value){
        console.log(value);
        receptenMarkup += '<div class="artikelCard"><div class="cardSpacer"><div class="cardContent">';        
        receptenMarkup += '<h2>'+ value.title +'</h2>';
        receptenMarkup += '<p>'+ value.tekstPreview +'</p>';		
        receptenMarkup += '<img src="'+ value.image +'" alt="'+ value.title +'">';
        receptenMarkup += ' <button>Volledig Artikel</button>';
        receptenMarkup += '</div></div></div>';    
    });
    $('.artikelCardHolder').append(artikellen_markup);
});
